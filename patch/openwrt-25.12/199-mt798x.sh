...
WIFINAMEE=$(echo "$FRPNAME" | tr -d ':' | tr 'a-f' 'A-F' | grep -o '.\{4\}$')
# 设置所有网口可访问网页终端
...
sed -i '$a https://mirrors.pku.edu.cn/openwrt/releases/25.12.5/targets/mediatek/filogic/kmods/6.12.94-1-5a6c1f71be683ae9980b15d3ce73e24d/packages.adb' /etc/apk/repositories.d/distfeeds.list
sed -i '$a https://mirrors.pku.edu.cn/openwrt/releases/25.12.5/targets/mediatek/filogic/packages/packages.adb' /etc/apk/repositories.d/distfeeds.list
...
uci set wireless.default_radio0.ssid=WiFi-${WIFINAMEE}-2.4G
uci set wireless.default_radio1.ssid=WiFi-${WIFINAMEE}-5G
